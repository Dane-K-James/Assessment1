		...
		try (FileInputStream fis = new FileInputStream(f); XSSFWorkbook excelWorkbook = new XSSFWorkbook(fis);) {
			...
			for (int i = 0; i < rows; i++) {
				if (k == studentlist.get(i).getId()) {
					...
					logger.info("Physics:" + "\n\t" + "Mark:" + currentStudent.getPhyMark() + "\n\t" + "Grade:"
							+ currentStudent.getPhyGrade() + "\n\t" + "Grade point:" + currentStudent.getPhyPoint());
					logger.info("Maths:" + "\n\t" + "Mark:" + currentStudent.getMathsMark() + "\n\t" + "Grade:"
							+ currentStudent.getMathsGrade() + "\n\t" + "Grade point:"
							+ currentStudent.getMathsPoint());
					logger.info("Chemistry:" + "\n\t" + "Mark:" + currentStudent.getChemMark() + "\n\t"
							+ "Grade:" + currentStudent.getChemGrade() + "\n\t" + "Grade point:"
							+ currentStudent.getChempoint());
